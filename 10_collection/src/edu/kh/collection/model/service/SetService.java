...
public class SetService {
	...
	public void ex1() {
		...
		// remove(Object value) : Set에 저장된 객체 중 value가 같은 객체 제거     //____remove(int index) set은 index가 없음
		// -> boolean 반환
		// -> 제거되면 true / 안되면 false
		...
		// 1. Iterator (반복자)
		// - 컬렉션에서 제공하는 컬레션 객체 반복 접근자
		// -> 컬렉션에 제공된 객체를 임의의 순서로 하나씩 꺼내는 역할
		...
		System.out.println("-----------------------------");
		//2. 향상된 for문 _배열과 컬렉션에 모두 사용. set은 컬렉션.
		for( String temp : set) {
			System.out.println(temp);
		}
	...
	public void ex2() {
		 ...
		 *      -> Student 오버라이딩해야 함.
		 *      
		 * Hash라는 단어가 붙은 컬렉션이 중복을 판단하는 방법  _ hashCode필요
		 * -> Object.hashCode() 오버라이딩
		 * 
		 * Hash 함수 : 입력된 단어를 지정된 길이의 무작위 문자열로 변환하는 함수
		 * 				_ 암호화에서 사용되기도. _ 4자리 길이 : 1을 입력해도 4자리 문자열, 145를 입력해도 4자리 문자열 반환
		 * 
		 ...
		 * 								__ set은 equals, hash는 hashCode를 써서 중복을 판단하니, HashSet은 둘다
		 * 
		 * -> equals() 또는 hashCode() 오버라이딩 하면
	...
	public void ex3() {
		...
		int i2 = i1;  // int = Integer  _int에 Integer 대입
		...
		// Integer -> int의 Wrpper Class
		 
		while(lotto.size() < 6 ) {
			
			int ran = (int)(Math.random() * 45 + 1); // 1~45
					
//			lotto.add(new Integer(ran)) - deprecated 삭제예정  // 삭제선. 너무 옛날 코드라 저절로 삭제될 것임
			lotto.add(ran); //_중복된거는 들어가지 않음. set특징
