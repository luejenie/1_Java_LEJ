...
//		1) 서버의 IP주소와 서버가 정한 포트번호를 매개변수로 하여 클라이언트용 소켓 객체 생성
		...
		try {
			...
			// throws UnknownHostException : IP가 잘못되었을 때 발생하는 예외 
...
//			3) 보조 스트림을 통해 성능 개선
			br = new BufferedReader(new InputStreamReader(is));  //문자 기반 (바이트기반 (is))
			pw = new PrintWriter(os);
			
					
//			4) 스트림을 통해 읽고 쓰기
			String serverMessage = br.readLine();  
			// 서버 -> 클라이언트에게 보낸 메세지를 저장     _ 2022년 08월 26일 15:51:05 [서버 접속 성공] 서버가 보낸 메세지 입력 받음.
