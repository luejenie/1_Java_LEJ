...
public class Example1 {
	...
	public void ex1() {
		...
		// *** 업캐스팅 확인 ***
		Car c2 = new Truck();
		// 부모 참조변수 = 자식 객체(부모 부분만 참조) __부모처럼 보인다. 
...
//		 System.out.println( c1.toString() );
	...
	public void ex2() {
		// 다형성(업캐스팅), 동적 바인딩, 객체 배열 _객체배열이 참조 변수의 묶음
		Car[] arr = new Car[3];				
...
//		
	...
	public void ex3() {
		...
		// _연산자 순서 ( ) > . > 형변환   -> 괄호를 안 해주면 뒤에 .이 우선적이라 에러남
		...
		System.out.println( c1 instanceof Spark);  // true // c1이 참조하는 객체가 Spark인가  -> t/f
		...
		check(c2);
		check(c3);
		//_ 아래 check 메서드 만들어서 적용한 것
		
		
		
		// 다운 캐스팅을 잘못한 경우
//		((Truck) c3).loading();   //_ Car c3를 Truck으로 강제 형변환 후 Truck 기능 적용
		
		// 코드 상에 오류는 발생X
		// ClassCastException : 형변환 예외
		//    -> 다운 캐스팅이 잘못된 경우 발생
		// * 해결 방법 : instanceof 연산자로 다운캐스팅 할 타입이 맞는지 검사  _무조건 다운캐스팅xxx
	...
	public void ex4() {
		...
		// instanceof 연산자
		// 1) 참조하는 개체의 타입을 검사하는 연산자
		// 2) 참조하는 객체가 특정 타입을 상속 받았는지 검사
		
		Car c = new Spark(); //업 캐스팅 상태
		
		if(c instanceof Car) {     //_ 2) c가 Car타입을 상속 받았는가
			// c가 참조하는 객체는 Spark이지만
			// 업 캐스팅 상태이기 때문에 Car로도 인식됨.(다형성)
			
			System.out.println("부모 Car 타입입니다.");	//true가 나옴.	 
		} else {
			( (Spark) c ).dc();
		}
