...
import edu.kh.poly.ex2.model.vo.LEJCalculator;
...
public class AbstractionService {
	...
	public void ex1() {
		...
		// 추상 클래슨느 정말로 객체 생성이 안될까?  -> 안됨
//		Animal a = new Animal(); // 에러. Cannot instantiate the type Animal  _Animal 타입으로는 객체화할 수 없다.
...
//		p1.breath();
		...
		// __객체배열, 매개변수, 컬레션 등에 적용한다고 하면 장점이 발휘됨.
		...
		// arr[i] 인덱스에서 어떤 Animal 자식 객체가 있는지 모르지만
		// 다들 강제 오버라이딩한 breath()라는 공통적인 메서드를 가지고 았으므로
		// arr[i].breath(); 작성이 가능.
	...
	public void ex2() {
		...
		// KH k1 = new KH(); // 에러. Cannot instantiate the type KH  안됨.		
