...
public class WhileExample {
	...
	public void ex2() {
		...
		//int input = 0; //input을 0으로 초기화할때 while 조건식과 충돌이 일어나서 while문을 통째로 넘어감.
		
		int input = -1; 
		// while문 내에서 입력 받은 값을 저장할 변수
		
		// 입력받은 값이 0인 경우 라는 조건을 판별하기 위해
		// while문 보다 앞에 먼저 선언
	...
	public void ex3() {
		...
		// ',' 만들기 위해 boolean사용
		boolean flag = false;  // flag : 신호용 변수
		// true : 첫 번째가 아닌 주문
		// false : 첫 번째 주문		
		...
		while(input != 9) {
			...
			System.out.println(); //줄 바꿈
			  ...
			  //false(첫 주문)
			if(flag && (input >= 1 && input <= 4)) {  //true인 경우(첫 주문 X) 실행 + 메뉴를 잘 입력했을 때
				
				menu += ", ";
								
			} else {  //false인 경우(첫 주문) 실행
				flag = true;   // false가 true가 됨. 
			}
			...
			switch(input) {
			case 1 : 
//				if(flag) {     //이런식으로 매 케이스마다 넣어주는 방법도 있음.
//					menu += ", ";
//				}
...
//			}
	...
	public void ex4() {
		...
		// 2번 예제를 do-while로 변경
	      ...
	      int input; // 1회가 do에서 보장되어 있기 때문에 변수 선언 없어도 에러가 뜨지 않음.
	      			 // 예제2에서 복잡했던 부분이 해결됨.
