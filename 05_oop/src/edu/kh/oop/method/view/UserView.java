...
public class UserView {
	...
	// User 참조 변수 선언(아직 참조하는 객체가 없다)       __?
	private User user = null;  // 회원 가입정보를 담고 있는 User 객체
	
	// 로그인한 User 객체의 주소를 참조할 변수
	private User loginUser = null; 
	// null인 경우 == 로그인 X
	// null이 아닌 경우 == 로그인 O  (참조하지 않는 경우가 아닌 경우)
	...
	public void displayMenu() {
		...
		do {
			...
			switch(input) {
			case 1: this.signUp(); break;    //this는 안 써도 됨.
	...
	public void printUser() {
		...
		if(loginUser != null) {  //로그인o
			...
			if(loginUser.getUserGender() == 'M') {
				...
				System.out.println("성별 : " + gender );  // 남성/여성으로 출력
	...
	public void updateUser() {
		...
		if(loginUser != null) {  // 로그인 O   //이름,나이,성별만 수정 가능하도록
