...
//		String input = br.readLine();  //==sc.nextLine(); 과 같은 구문
		// public String readLine() throws IOException     //_ctrl+f2
		...
		for(int i=0 ; i <= arr.length /*실수로*/; i++) {
			//i = 0~4 (4는 인덱스 범위 초과)
	...
	public void ex2() {
		...
		try { 
			...
			System.out.println(input);//IOException (Checked)  _체크해야함
			
			// 예외 강제 발생
			// == 예외 객체를 생성해서 던짐
//			new IOException();  //예외 객체 만듦
			throw new IOException();  //예외 강제
	...
	public void ex3() {
		...
		try {
			...
			// 앞에서 모든 예외를 처리해버려서 여기까지 닿지 못함.
				
			
									// 부모타입 참조변수    //InputMismatch 자식 객체
			...
			// ex) sc.nextInt();를 통한 입력 시, 정수가 아닌 값을 입력하면 발생 __(정수가 아니라 '가'와 같은 char형, double형 등)
...
//		ex5(); // 빨간 줄에 마우스 놓고 자동완성 surround~~
		...
		try {
			...
			// _ 출력되는 빨간 내용 자체가 이 메서드 내용
			// _ 로그 추적해서 어디가 문제인지 봄
